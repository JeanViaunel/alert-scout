...
/**
 * Send market report via WhatsApp API
 */
async function sendWhatsAppMessage(phone: string, message: string) {
  ...
  if (!token) {
    console.warn('⚠️ WHATSAPP_TOKEN not found in environment. Skipping message send.');
    return;
  }
...
export async function scheduleWeeklyReports() {
  ...
  for (const user of users) {
    ...
    for (const district of districts) {
      ...
      console.log(`\n📱 Sending WhatsApp report for ${district} to ${user.phone}...`);
