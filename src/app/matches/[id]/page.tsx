...
function PriceHistorySection({ matchId, currentPrice, createdAt }: { matchId: string; currentPrice: number; createdAt: string }) {
  ...
  const formattedHistory = history.length > 0 ? history.map(h => ({
    date: new Date(h.scrapedAt).toLocaleDateString(),
    price: h.price
  })) : [
    { date: new Date(createdAt).toLocaleDateString(), price: currentPrice }
  ];
...
export default function MatchDetailPage() {
  ...
  const specs = [
    ...
    match.commuteTime && {
      icon: Clock,
      label: `${match.commuteTime} min commute`,
    },
            ...
            <div className="space-y-6">
              ...
              <FadeIn delay={0.28}>
                <PriceHistorySection 
                  ...
                  createdAt={match.createdAt as unknown as string} 
                />
              </FadeIn>
              ...
              {isProperty && (
                <FadeIn delay={0.29}>
                  <InvestmentCalculator initialPrice={match.price} matchId={match.id} />
                </FadeIn>
              )}
